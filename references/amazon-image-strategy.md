...
## Category-specific carousel patterns

Category-specific visual systems — buyer-concern priority order, carousel patterns,
scene/prop/persona language, safety notes — live in `references/categories/`. Load the
matching profile (e.g. `categories/fitness-equipment.md` for power towers, dip bars,
benches, and racks) and apply it on top of the generic rules in this file. Do not
transplant one category's props, scenes, or personas onto another category's product.
...
Category-specific priority orders, image pools, and high-impact visual plans live
in `references/categories/` — see `fitness-equipment.md` for the worked example.
